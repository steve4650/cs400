public class BSTRotation<T extends Comparable<T>> extends BinarySearchTree<T> {
  ...
  protected void rotate(BinaryNode<T> child, BinaryNode<T> parent) {
    ...
    // Update grandparent <--> child link
    BinaryNode<T> grandparent = parent.up();
    if (grandparent != null) {
      if (grandparent.downLeft() == parent) {
  ...
  public boolean test3() {
    BSTRotation<String> stringTree = new BSTRotation<String>();
    stringTree.add("zoe");
    stringTree.add("roe");
    // left rotation
    stringTree.rotate(stringTree.root.downLeft(), stringTree.root);
    if (!(stringTree.root.getEntry() == "roe" && stringTree.root.downLeft().getEntry() == "zoe")) {
      return false;
    }
    return true;
  }
